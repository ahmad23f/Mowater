...
class UserServices {
  ...
  static saveUserAccountRememberMe(
      {required String email, required String password}) async {
    final userBox = await Hive.openBox<User>('RememberMeAccount');

    if (userBox.containsKey('user')) {
      userBox.delete('user'); // Remove the old user
    }
    final user = User()
      ..email = email
      ..password = password;
    userBox.put('user', user); // Save the new user
  }
}
